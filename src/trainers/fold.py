...
class FDUTrainer(BaseTrainer):
    ...
    def set_0_update_table(self):
        ...
        for i in range(len(self.clients)):
            ...
            self.nn_record.append(temp)
        # self.client_update_fold()
    def set_0_update_table_fold(self):
    ...
    # def client_update_fold(self):
    #     self.local_train_fold(0,self.clients,self.nn_record,self.nn)
    def initiali0ze_update_table_varp(self):
        solns, _ = self.local_train(0, self.clients)
    ...
    def train(self):
        ...
        # self.latest_model = self.worker.get_flat_model_params_fold(self.nn).detach()
        self.denomi_list = []
        table_initialized = True
        self.set_0_update_table_fold()
        # self.client_update_fold()
        ##############
        truth_round = 0
        ...
        for round_i in range(self.num_round):
            ...
            if table_initialized:
                solns, stats, self.nn_record = self.local_train_fold2(truth_round, self.selected_clients,
                                                                             self.nn_record, self.nn)
                # Solve minimization locally
                # solns, stats = self.local_train(truth_round, self.selected_clients)
                # solns, stats = self.local_train_fold(truth_round, self.selected_clients,self.nn_record,self.nn)
                #
                ...
                self.latest_model=self.aggregate_fold()
                ...
                train_loss, train_acc = self.evaluate_train_fold(self.nn)
                test_loss, test_acc = self.evaluate_test_fold(self.nn)
    ...
    def aggregate_fold(self):
        x = {}
        c = {}
        for k, v in self.nn_record[0].named_parameters():
            x[k] = torch.zeros_like(v.data)
            c[k] = torch.zeros_like(v.data)

        for (idx, client) in enumerate(self.selected_clients):
            for k, v in self.nn_record[client.cid].named_parameters():
                x[k] += self.nn_record[client.cid].delta_y[k] / len(self.selected_clients)
                c[k] += self.nn_record[client.cid].delta_control[k] / len(self.selected_clients)

        for k, v in self.nn.named_parameters():
            v.data += x[k].data

            self.nn.control[k].data += c[k].data * (len(self.selected_clients) / len(self.clients))
