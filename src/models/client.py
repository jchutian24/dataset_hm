...
class Client(object):
    ...
    def local_test(self, **kwargs):
        ...
        local_solution, worker_stats = self.worker.local_test(self.test_dataloader, **kwargs)
    ...
    def local_train_fold(self,ann,server, **kwargs):
        """Solves local optimization problem
        ...
        local_solution, worker_stats,step = self.worker.local_train_fold(self.train_dataloader,ann,server, **kwargs)#ann[k]
        # ann.train()
        stats = {'id': self.cid}
        stats.update(worker_stats)

        return (len(self.train_data), local_solution), stats,step
    ...
    def evaluate_train_fold(self, nn, **kwargs):
        return self.worker.evaluate_fold(self.train_dataloader,nn, **kwargs)

    def evaluate_test_fold(self, nn,**kwargs):
        return self.worker.evaluate_fold(self.test_dataloader,nn, **kwargs)